#Using the fucking awk to parse dump stack lines embedded in a kernel
#log to a more readable format

#Get the absolute path of vmlinux
vmpath=`readlink -f $2`

#There is a case when .dump_temp folder already exists,used or  
#generated by others, however, I don't care, for this script is
#used by nobody except me, I'll be careful with it
rm -r .dump_temp
#Create a temp directory as a working place, seems not so elegant.
mkdir .dump_temp
cp $1 .dump_temp
cd .dump_temp
...
#mateched line, but this is fucking slow because everytime I
#use addr2line, it has to read vmlinux, which is often several
